...
public class ThunderCrashAbility {
    ...
    public static void execute(Player player, AbilityData data, ServerLevel level) {
        ...
        for (double angle = 0; angle < Math.PI * 2; angle += Math.PI / 20) {
            ...
            level.sendParticles(ParticleTypes.ELECTRIC_SPARK, px, player.getY() + 0.2, pz, 2, 0.0, 0.1, 0.0, 0.0);
        }
        level.sendParticles(ParticleTypes.ELECTRIC_SPARK,
                player.getX(), player.getY() + 0.5, player.getZ(), 60, 0.3, 0.1, 0.3, 0.3);
        ...
        for (LivingEntity target : targets) {
            ...
            // Use setPos() — moveTo() was removed in 1.20+
