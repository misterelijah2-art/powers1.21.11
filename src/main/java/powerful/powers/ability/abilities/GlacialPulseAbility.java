...
public class GlacialPulseAbility {
    ...
    public static void execute(Player player, AbilityData data, ServerLevel level) {
        ...
        int    amplifier = fullCharge ? 3 : 1;
        int    duration  = fullCharge ? 100 : 60;
        ...
        level.playSound(null, player.blockPosition(), SoundEvents.POWDER_SNOW_PLACE,
        ...
        for (LivingEntity target : targets) {
            // MobEffects.SLOWNESS is the correct name in 1.21.1
    ...
    public static void doPassive(Player player, ServerLevel level, long tick) {
        ...
        double px = player.getX() + Math.cos(angle) * r;
        double pz = player.getZ() + Math.sin(angle) * r;
        level.sendParticles(ParticleTypes.SNOWFLAKE,    px, player.getY() + 0.1, pz, 1, 0.0, 0.02, 0.0, 0.0);
        level.sendParticles(ParticleTypes.ITEM_SNOWBALL, px, player.getY() + 0.3, pz, 1, 0.0, 0.01, 0.0, 0.0);
        if (tick % 50 == 0)
            level.playSound(null, player.blockPosition(), SoundEvents.POWDER_SNOW_STEP,
